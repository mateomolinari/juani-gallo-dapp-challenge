...
import ABI from "../ABIs/compoundABI.json";
import ERC20ABI from "../ABIs/ERC20.json";
import { useEthers } from "@usedapp/core";

function MainUI() {
  const compoundAddress: string = "0xF0d0EB522cfa50B716B3b1604C4F0fA6f04376AD";
  const [userBalance, setUserBalance] = useState<ethers.BigNumber>();
  const [network, setNetwork] = useState<string>("");
  const [DAIallowed, setDAIAllowed] = useState<boolean>(false);
  const [mining, startTransaction] = useState<boolean>(false);
  ...
  const mintcDAI = async () => {
    let tx = await CErc20Delegator.mint(
        ...
        {!mining ? (
          ...
          >
            ...
            <Text fontSize="xl" color="white">
              cDAI Balance:{" "}
              {userBalance
                ? parseFloat(ethers.utils.formatUnits(userBalance, 8)).toFixed(
                    2
                  )
                : "0"}
            </Text>
